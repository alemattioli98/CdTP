...
// Funzione di parsing intelligente per la datazione
const parseDatingString = (input) => {
    if (!input || typeof input !== 'string') return "";
    const text = input.toLowerCase().trim();

    // Priorità 1: Cerca termini testuali espliciti
    if (text.includes("seconda metà vii")) return "seconda metà VII sec. a.C.";
    if (text.includes("primo quarto vi")) return "primo quarto VI sec. a.C.";
    if (text.includes("secondo quarto vi")) return "secondo quarto VI sec. a.C.";
    if (text.includes("terzo quarto vi")) return "terzo quarto VI sec. a.C.";
    if (text.includes("ultimo quarto vi")) return "ultimo quarto VI sec. a.C.";
    if (text.includes("prima metà vi")) return "prima metà VI sec. a.C.";
    if (text.includes("seconda metà vi")) return "seconda metà VI sec. a.C.";
    if (text.includes("primo quarto v")) return "primo quarto V sec. a.C.";
    if (text.includes("secondo quarto v")) return "secondo quarto V sec. a.C.";
    if (text.includes("terzo quarto v")) return "terzo quarto V sec. a.C.";
    if (text.includes("ultimo quarto v")) return "ultimo quarto V sec. a.C.";
    if (text.includes("prima metà v")) return "prima metà V sec. a.C.";
    if (text.includes("seconda metà v")) return "seconda metà V sec. a.C.";
    
    // Priorità 2: Cerca range numerici (es. 520-510 a.C.)
    const rangeMatch = text.match(/(\d{3,4})\s*-\s*(\d{3,4})/);
    if (rangeMatch) {
        const start = parseInt(rangeMatch[1]);
        const end = parseInt(rangeMatch[2]);
        const midPoint = (start + end) / 2;
        // Usa il punto medio per determinare il periodo più probabile
        if (midPoint >= 575 && midPoint <= 600) return "primo quarto VI sec. a.C.";
        if (midPoint >= 550 && midPoint < 575) return "secondo quarto VI sec. a.C.";
        if (midPoint >= 525 && midPoint < 550) return "terzo quarto VI sec. a.C.";
        if (midPoint >= 500 && midPoint < 525) return "ultimo quarto VI sec. a.C.";
        if (midPoint >= 475 && midPoint < 500) return "primo quarto V sec. a.C.";
        if (midPoint >= 450 && midPoint < 475) return "secondo quarto V sec. a.C.";
        if (midPoint >= 425 && midPoint < 450) return "terzo quarto V sec. a.C.";
        if (midPoint >= 400 && midPoint < 425) return "ultimo quarto V sec. a.C.";
        if (midPoint >= 625 && midPoint < 650) return "terzo quarto VII sec. a.C."; // Corrected logic: 650-625 BC
        if (midPoint >= 600 && midPoint < 625) return "ultimo quarto VII sec. a.C."; // Corrected logic: 625-600 BC
    }

    // Priorità 3: Cerca una data singola (es. 520 a.C.)
    const singleDateMatch = text.match(/(\d{3,4})/);
    if (singleDateMatch) {
        const year = parseInt(singleDateMatch[1]);
        if (year >= 575 && year <= 600) return "primo quarto VI sec. a.C.";
        if (year >= 550 && year < 575) return "secondo quarto VI sec. a.C.";
        if (year >= 525 && year < 550) return "terzo quarto VI sec. a.C.";
        if (year >= 500 && year < 525) return "ultimo quarto VI sec. a.C.";
        if (year >= 475 && year < 500) return "primo quarto V sec. a.C.";
        if (year >= 450 && year < 475) return "secondo quarto V sec. a.C.";
        if (year >= 425 && year < 450) return "terzo quarto V sec. a.C.";
        if (year >= 400 && year < 425) return "ultimo quarto V sec. a.C.";
        if (year >= 625 && year < 650) return "terzo quarto VII sec. a.C."; // Corrected logic: 650-625 BC
        if (year >= 600 && year < 625) return "ultimo quarto VII sec. a.C."; // Corrected logic: 625-600 BC
    }

    // Se nessuna corrispondenza, restituisce il testo originale per non perdere dati
    return input;
};
...
export default function AddArtifact() {
  ...
  useEffect(() => {
    const fetchTombs = async () => {
      try {
        const tombList = await Tomb.list('Numero_tomba');
        setTombs(tombList);
      } catch (error) {
        console.error("Failed to fetch tombs:", error);
      }
    };
    fetchTombs();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSaving(true);
    try {
      // Normalizza la datazione prima di salvare
      const normalizedDating = parseDatingString(artifactData.dating);

      const dataToSave = { 
          ...artifactData, 
          dating: normalizedDating 
      };
      ...
      console.log('Saving artifact data:', dataToSave);
      
      await Artifact.create(dataToSave);
      alert("Reperto creato con successo!");
    ...
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Nuovo Reperto</h1>
            <p className="text-amber-800 mt-1">Compila la scheda di catalogazione scientifica.</p>
          </div>
        </div>
        <button type="button" onClick={() => navigate(createPageUrl("Artifacts"))} className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-800 rounded-lg shadow-sm hover:bg-gray-200 transition-colors">
          ...
          <FormField label="Datazione">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Inserisci una datazione testuale. Verrà normalizzata al salvataggio.
              </p>
            </div>
          </FormField>
        </div>
      ...
      <div className="flex justify-end pt-4">
        <button 
          type="submit" 
          disabled={isSaving} 
          className="flex items-center justify-center gap-2 px-8 py-3 bg-amber-600 text-white font-bold rounded-lg shadow-lg hover:bg-amber-700 disabled:bg-amber-300 disabled:cursor-not-allowed transition-all hover:scale-105 w-44"
        >
          {isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : "Salva Reperto"}
        </button>
      </div>
    </form>
    </div>
